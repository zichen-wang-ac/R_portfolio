...
##TO DO
# 1. correctly plot from lines 674 to 679 plot with monthly changes in Tfl bike rentals faceted by year.
# 2. add the green and red areas for values of excessive usage depending if under or over the expected results.

# Using the mean, the average number of bikes hired in each month is shown below. 
mean_expected_monthly_rides <- bike %>%
  group_by(month) %>%
  summarise(mean_expected_monthly_rides = mean(bikes_hired))
  
  mean_expected_monthly_rides

# The actual mean of bike rentals are calculated.    
mean_actual_monthly_rides <- bike %>% 
  rename(actual_rentals = bikes_hired) %>% 
  group_by(year,month) %>% 
  summarise(mean_actual_rides = mean(actual_rentals))
  
mean_actual_monthly_rides

# A plot of the mean number of bikes hired in each month is shown below to compare with original plot to replicate.
  mean_plot <- mean_expected_monthly_rides %>% 
    ggplot(data = mean_expected_monthly_rides, mapping = aes(x = month, y = mean_expected_monthly_rides, group=1)) +
    geom_line() +
    labs(title = "Mean monthly TfL bike rentals", y = "Bike Rentals") + 
    theme(legend.position = "none") +
  NULL
...
excess_rentals_table <- left_join(mean_actual_monthly_rides,mean_expected_monthly_rides,by="month") %>% 
  mutate(excess_rentals = mean_actual_rides - mean_expected_monthly_rides)
  
#include <- c("2015", "2016", "2017", "2018", "2019", "2020")
plot_A <- excess_rentals_table %>% 
  ggplot(data = excess_rentals_table, mapping = aes(x=month, y = mean_actual_rides))+
  geom_point()+
  #facet_wrap(~year%in%include)
...
# A plot of the median number of bikes hired in each month is shown below to compare with original plot to replicate.
    ...
    ggplot(data = median_expected_monthly_rides, mapping = aes(x = month, y = median_expected_monthly_rides, group=1)) +
    geom_line() +
    labs(title = "Median monthly TfL bike rentals", y = "Bike Rentals") + 
    theme(legend.position = "none") +
  NULL
